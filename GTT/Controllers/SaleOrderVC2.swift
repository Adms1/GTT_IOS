...
class SaleOrderVC2: UIViewController {
    ...
    @IBAction func btnClickSupplier(_ sender:UIButton)
    {
        ...
        dropDown.selectionAction = { [weak self] (index: Int, item: String) in //8
          guard let _ = self else { return }
            ...
            self!.supplierName = self?.arrSupplierList[index].SupplierName ?? ""
...
extension SaleOrderVC2:supplierDelegate,paymentDelegate,delivaryDelegate
{
    func delivaryMethod() {
        callMultipleApiCAlling()
    }
...
  
        ApiHttpUtility.sharedUHttp.callGetMultipleApiHttpUtility(urlString: API.GetSoGridNumberGridYear) { (responceJson, error) in
            if(responceJson?[Constants.Status] == "Success")
            {
                print(responceJson)
                let arrData = responceJson?["Data"].array
