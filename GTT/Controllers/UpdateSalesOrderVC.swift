...
class UpdateSalesOrderVC: UIViewController {
...
//    var arrPlace = [String]()
    ...
    var objPurchaseOrder:saleOrderListModel?
    ...
    override func viewDidLoad() {
        ...
        txtGridNumber.text = objPurchaseOrder?.GridNumber ?? ""
        txtGridYear.text = objPurchaseOrder?.GridYear ?? ""

        btnSupplier.setTitle(objPurchaseOrder?.SupplierName ?? "", for: .normal)
        supplierName = objPurchaseOrder?.SupplierName ?? ""
        txtSupplierContract.text = objPurchaseOrder?.BuyerContractID ?? ""
        btnEquitment.setTitle(objPurchaseOrder?.EquipmentTypeName ?? "", for: .normal)
        btnPortLoading.setTitle(objPurchaseOrder?.PortOfLoading ?? "", for: .normal)
        btnPortDischange.setTitle(objPurchaseOrder?.PortOfDischarge ?? "", for: .normal)
        btnPlaceDelivary.setTitle(objPurchaseOrder?.FinalPlaceOfDelivery ?? "", for: .normal)

        btnPaymentsTerms.setTitle(objPurchaseOrder?.PaymentTerm ?? "", for: .normal)
        btnDelivaryTerms.setTitle(objPurchaseOrder?.DeliveryTerm ?? "", for: .normal)

        equipmentTypeID = objPurchaseOrder!.EquipmentTypeID
        btnEquitment.setTitle(objPurchaseOrder?.EquipmentTypeName ?? "", for: .normal)

        supplierID = objPurchaseOrder?.BuyerID ?? 0

        txtEquitpmentCount.text = "\(objPurchaseOrder?.EquipmentCount ?? 0)"

        txtTolerance.text = objPurchaseOrder?.Tolerance ?? ""

        btnCountryOrigin.setTitle(objPurchaseOrder?.CountryOfOrigin ?? "", for: .normal)
        btnContryLoading.setTitle(objPurchaseOrder?.CountryOfOrigin ?? "", for: .normal)
        btnPortLoading.setTitle(objPurchaseOrder?.PortOfLoading ?? "", for: .normal)
        btnCountryDelivary.setTitle(objPurchaseOrder?.CountryOfDischarge ?? "", for: .normal)
        btnPortDischange.setTitle(objPurchaseOrder?.PortOfDischarge ?? "", for: .normal)

        btnPlaceDelivary.setTitle(objPurchaseOrder?.FinalPlaceOfDelivery ?? "", for: .normal)
        countryFindPlaceId = objPurchaseOrder?.FinalPlaceOfDeliveryID ?? 0
        paymentTermID = objPurchaseOrder?.PaymentTermID ?? 0
        delivaryTermId = objPurchaseOrder?.DeliveryTermID ?? 0

        PortofDischargeID = objPurchaseOrder?.PortOfDischargeID ?? 0
        strCountryDeliPortDischarge = objPurchaseOrder?.CountryOfDischargeID ?? 0
        strCountryLoading = objPurchaseOrder?.CountryOfOriginID ?? 0
        strCountryPortLoadingId = objPurchaseOrder?.PortOfLoadingID ?? 0
...
extension UpdateSalesOrderVC:supplierDelegate,paymentDelegate,delivaryDelegate
{
    func delivaryMethod() {
        callMultipleApiCAlling()
    }
...
//        }
      ...
      guard let userId = UserDefaults.standard.value(forKey: "logindata") else
        ...
        var params = [String : Any]()
        params = ["UserID":userId,"SalesOrderID":objPurchaseOrder?.SalesOrderID ?? 0,"GRIDNumber":txtGridNumber.text!,"GRIDYear":txtGridYear.text!,"BuyerID":supplierID,"BuyerContractID":txtSupplierContract.text ?? 0,"EquipmentTypeID":equipmentTypeID,"Tolerance":txtTolerance.text ?? 0,"CountryOfOriginID":strCountryLoading,"PortofLoadingID":strCountryPortLoadingId,"PortofDischargeID":PortofDischargeID,"FinalPlaceofDeliveryID":countryFindPlaceId,"PaymentTermID":paymentTermID,"DeliveryTermID":delivaryTermId,"CountryofDischargeID":strCountryDeliPortDischarge ,"EquiptmentCount":txtEquitpmentCount.text!,"AdvanceType":"0","AdvanceAmount":"0"]

        print("paramsparamsparamsparams",params)

        ApiHttpUtility.sharedUHttp.callPostHttpUtility(urlString: API.AddSalesOrder, params: params) { (responceJson) in
            if(responceJson[Constants.Status] == "Success")
            {
                MBProgressHUD.hide(for: self.view, animated: true)
...
//                userDefaults.synchronize()
                ...
                let storyboard = UIStoryboard(name: "Main", bundle: nil)
                let vc = storyboard.instantiateViewController(withIdentifier: "SaleOrderVC1") as! SaleOrderVC1
                vc.salesOrderId = responceJson["Data"].intValue
                vc.supplierId = self.supplierID
                vc.supplierName = self.supplierName
