...
class UpdateSalesPopUpVC: UIViewController {
    ...
    var objPurchaseOrder:saleOrderMatrialModel?
...
extension UpdateSalesPopUpVC
{
    ...
    func AddPurchaseMaterialApiCall()
    {
        ...
        params = ["SalesOrderMaterialID":objPurchaseOrder?.SalesOrderMaterialID ?? 0,"SalesOrderID":purchaseOrderId,"MaterialID":materialId,"ContractQuantity":txtQty.text ?? 0,"RatePerMT":txtRate.text ?? 0,"CurrencyID":currancyId as Any,"AverageLoading":txtAvgLoading.text ?? 0,"PackagingID":packagingId]

        ApiHttpUtility.sharedUHttp.callPostHttpUtility(urlString: API.AddSalesOrderMaterial, params: params) { (responceJson) in
...
extension UpdateSalesPopUpVC:AddMaterialDelegate,AddCurrencyDelegate,packageDelegate
{
