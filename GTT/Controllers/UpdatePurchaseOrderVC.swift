...
class UpdatePurchaseOrderVC: UIViewController {
...
//    var arrPlace = [String]()
    ...
    var objPurchaseOrder:purchaseOrderListModel?
...
//        btnSubmit.layer.masksToBounds = true
        ...
        txtSupplierContract.text = objPurchaseOrder?.SupplierContractID ?? ""
        ...
        equipmentTypeID = objPurchaseOrder?.EnquipmentTypeID ?? 0
        btnEquitment.setTitle(objPurchaseOrder?.EquipmentTypeName ?? "", for: .normal)

        supplierID = objPurchaseOrder?.SupplierID ?? 0
...
extension UpdatePurchaseOrderVC{
    ...
    @IBAction func btnClickPaymnetTermList(_ sender:UIButton)
    {
        ...
        dropDown.selectionAction = { [weak self] (index: Int, item: String) in //8
          guard let _ = self else { return }
           // self!.strCountryPortLoadingId = self?.arrPortList[index].countryID ?? 0
...
extension UpdatePurchaseOrderVC:supplierDelegate,paymentDelegate,delivaryDelegate
{
    func delivaryMethod() {
        callMultipleApiCAlling()
    }
...
//                userDefaults.synchronize()
              ...
              //  vc.updatePurchaseOrder = "Update Purchase Order"
